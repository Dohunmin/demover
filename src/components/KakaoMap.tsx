              ...
              data.forEach((place: any) => {
                ...
                const svgContent = `
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32" height="32">
                    ...
                    <circle cx="16" cy="16" r="14" fill="#FF6B6B" stroke="white" stroke-width="2" filter="url(#shadow)"/>
                    <text x="16" y="20" text-anchor="middle" font-size="12" fill="white">📍</text>
                  </svg>
                `;
                const markerSvg = `data:image/svg+xml;utf8,${encodeURIComponent(svgContent)}`;
                const markerImage = new window.kakao.maps.MarkerImage(markerSvg, imageSize, imageOption);
