...
-- 기존 함수 삭제 후 새로 생성
DROP FUNCTION IF EXISTS public.tour_area_list(integer, integer, text);
DROP FUNCTION IF EXISTS public.tour_pet_list(integer, integer);

-- 일반 관광지 목록 함수
CREATE OR REPLACE FUNCTION public.tour_area_list(page_no integer DEFAULT 1, rows integer DEFAULT 10, keyword text DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- 반려동물 동반 관광지 목록 함수
CREATE OR REPLACE FUNCTION public.tour_pet_list(page_no integer DEFAULT 1, rows integer DEFAULT 100)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
